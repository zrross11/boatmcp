...
class DockerfileGenerator:
    ...
    def _generate_python_dockerfile(self, analysis: ProjectAnalysis, request: DockerfileGenerationRequest) -> DockerfileTemplate:
        ...
        if analysis.dependencies and any(dep in analysis.dependencies for dep in ["pillow", "psycopg2", "lxml", "numpy", "scipy"]):
        ...
        if analysis.config_files and any(f.path.name in ["requirements.txt", "pyproject.toml"] for f in analysis.config_files):
            instructions.extend([
    ...
    def _generate_java_dockerfile(self, analysis: ProjectAnalysis, request: DockerfileGenerationRequest) -> DockerfileTemplate:
        ...
        if analysis.config_files and any(f.path.name == "pom.xml" for f in analysis.config_files):
            instructions.extend([
                DockerfileInstruction("RUN", "./mvnw clean package"),
                DockerfileInstruction("EXPOSE", "8080"),
                DockerfileInstruction("CMD", '["java", "-jar", "target/*.jar"]')
            ])
        elif analysis.config_files and any(f.path.name == "build.gradle" for f in analysis.config_files):
            instructions.extend([
